...
static  NSMutableArray <NSDictionary *> *ZJToastArray = nil;
static  BOOL ZJToastIsShowing = NO;
...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position {
    ...
    [self makeQueueToast:message duration:interval position:position];
}
...
- (void)showToast:(UIView *)toast duration:(CGFloat)interval position:(id)point {
    ...
    NSDictionary *dic = @{@"toast" : toast,
                          @"interval" : @(interval),
                          @"point" : point};
    if (!ZJToastArray) {
        ZJToastArray = [NSMutableArray array];
    }
    
    [ZJToastArray addObject:dic];
    [self showZJToast];
}


- (void)showZJToast {
    if (ZJToastArray.count < 1 || ZJToastArray) {
        return;
    }
    
    ZJToastIsShowing = YES;
    NSDictionary *dic = ZJToastArray[0];
    UIView *toast = dic[@"toast"];
    CGFloat interval = [dic[@"interval"] floatValue];
    id point = dic[@"point"];
                     ...
                     animations:^{
                         ...
                         [UIView animateWithDuration:CSToastFadeDuration
                                          ...
                                          animations:^{
                                              ...
                                              [ZJToastArray removeObjectAtIndex:0];
                                              ZJToastIsShowing = NO;
                                              [self showZJToast];
                                          }];
                     }];
